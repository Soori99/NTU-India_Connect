...
for j=1:nog+1
    ...
    [points, cost] = Traj_MOGWO(finalcentroidofgates(j,1:2),finalcentroidofgates(j+1,1:2),leftgate(j,1:2),rightgate(j,1:2),leftgate(j+1,1:2),rightgate(j,1:2),finalobs,USVvelocity,option,headingangle,nVar);
    for i=1:nVar
...
if drawing_map ==1
    ...
    legend(subset,'Entrance/Exit - Left Blue Gate','Entrance/Exit - Right Red Gate','Boundary','Obstacle','WP - Waypoint','SP - Starting Point','EP - End Point','Green Gate - Left','Red Gate - Right','Traj-MOGWO')
