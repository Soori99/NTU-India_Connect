...
for k= 1:nVar
    r(1,k) = unifrnd(0.6,0.8);
end
%% MOIGWO main loop

for it=1:MaxIt
    a=2-2*((1/(exp(1)-1))*(exp(it/MaxIt)-1));
    for i=1:GreyWolves_num
        ...
        for k = 1:nVar
            rk(1,k) = f*(r(1,k))^2*sin(pi*r(1,k)); %Chaotic Factor
        end
        ...
        b1=1; %Social Harmony
        b2=1; %Individual Memory

        for k= 1:nVar
            rk(1,k) = f*(rk(1,k))^2*sin(pi*rk(1,k));
        end
        c = b2.*rk;
        t = c.*(GreyWolves(i).Best.Position - GreyWolves(i).Position); %Individual Memory
        GreyWolves(i).Position= b1*(X1+X2+X3)./3 + t;%Updating Grey Wolf using alpha,beta, delta 
...
title('Optimal Solution Pareto Set using MOIGWO');
legend('MOIGWO');
xlabel('F_1');
ylabel('F_2');
...
display(['The IGD Metric obtained by MOIGWO is     : ', num2str(M_IGD)]);
display(['The GD Metric obtained by MOIGWO is      : ', num2str(M_GD)]);
display(['The HV Metric obtained by MOIGWO is      : ', num2str(M_HV)]);
display(['The Spacing Metric obtained by MOIGWO is : ', num2str(M_Spacing)]);
display(['The Spread Metric obtained by MOIGWO is  : ', num2str(M_Spread)]);
display(['The DeltaP Metric obtained by MOIGWO is  : ', num2str(M_DeltaP)]);
