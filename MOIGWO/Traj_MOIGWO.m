function [FinalBestPath, FinalBestCost] = Traj_MOIGWO(startp,endp,startpgreen,startpred,endpgreen,endpred,obs,USVvelocity,option,headingangle,nVar)
 ...
 for i=1:GreyWolves_num
    ...
    for k= 1:nVar
        r(k,1) = unifrnd(0.6,0.8);
    end

    %% MOIGWO Main Loop
    
    for ti=1:Tmax
        a=2-2*((1/(exp(1)-1))*(exp(ti/Tmax)-1));
...
%             Choose the alpha, beta, and delta grey wolves
            ...
            for k = 1:nVar
                rk(k,1) = f*(r(k,1))^2*sin(pi*r(k,1)); %Chaotic Factor
            end
            ...
            for k= 1:nVar
                rk(k,1) = f*(rk(k,1))^2*sin(pi*rk(k,1));
            end
            ...
            g = b2.*rand(nVar,1);
            t = g.*(GreyWolves(i).Best.Position - GreyWolves(i).Position); %Individual Memory
